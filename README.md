...
있습니다. LLM 기반 조건 추출은 실제 모델 대신 구조화 JSON을 반환하는 가짜 채팅
모델로 검증해, 테스트가 네트워크 상태와 모델 응답 변동에 영향을 받지 않도록
했습니다.
...
상품 정보만으로 최종 답변을 생성합니다. LLM과 prompt는 시작 시 한 번 준비해 대화
턴마다 재사용하며, 콘솔 실행 진입점도 이 파일에 있습니다.
