...
사용자가 원하는 상품 유형, 기간, 금리 기준, 가입 조건을 입력하면 OpenAI 채팅 모델이 대화에서 추천 조건을 구조화해 추출합니다. 이후 정제된 상품 데이터에서 조건에 맞는 상품을 찾고, 검색된 상품 정보만 바탕으로 자연어 답변을 생성합니다.
...
- LLM 기반 사용자 조건 추출과 멀티턴 상태 병합
- 구조화 조건 필터와 의미 검색을 결합한 상품 검색
- 부가 조건 하나를 완화했을 때 더 높은 금리 상품이 있는지 대안 탐색
...
├── preferences.py           # LLM 기반 사용자 추천 조건 추출
├── retrieval.py             # 상품 필터, 금리 정렬, 의미 검색과 대안 탐색
...
사용자 대화
→ preferences.py LLM 기반 조건 추출
→ retrieval.py 구조화 필터 + Chroma 의미 검색 + 단일 조건 완화 대안 탐색
→ graph_chat.py 답변 생성
→ 콘솔 또는 app.py 웹
```

추천 단계에서는 사용자의 원래 조건을 모두 만족하는 상품을 먼저 고릅니다.
그 다음 상품 유형과 기간 같은 핵심 조건은 유지하고, 카드 조건, 급여이체,
자동이체, 모바일 가입 선호 같은 부가 조건만 한 번에 하나씩 완화해 더 높은
금리 상품이 있는지 확인합니다. 완화 상품은 메인 추천에 섞지 않고 별도 대안으로
전달하며, 어떤 조건을 완화해야 하는지와 현재 최선 상품 대비 금리 개선폭은
Python 코드에서 계산합니다.
...
- `OPENAI_API_KEY`: 임베딩 생성, 조건 추출과 챗봇 답변 생성에 사용합니다.
- `OPENAI_CHAT_MODEL`: 선택값입니다. 설정하지 않으면 `gpt-5.6-luna`를 사용합니다.
...
조건 추출, 상품 검색, 조건 완화 대안 탐색과 웹 API 오류 처리는 외부 API를
호출하지 않고 테스트할 수 있습니다. LLM 기반 조건 추출은 실제 모델 대신 구조화
JSON을 반환하는 가짜 채팅 모델로 검증해, 테스트가 네트워크 상태와 모델 응답
변동에 영향을 받지 않도록 했습니다.
...
최근 사용자 발화와 기존 대화 상태를 OpenAI 채팅 모델에 전달해 상품 유형, 기간,
금리 기준, 월 납입액과 우대조건 수용 여부를 JSON 형태로 추출합니다. 추출 결과는
허용된 값만 검증해 LangGraph 상태에 병합하고, 모델이 `null`로 반환한 항목은
이전 대화 값을 유지합니다.

기존 규칙 기반 `parse_*` 함수들은 현재 그래프의 주 추출 경로가 아닙니다. LLM
전환 전 기준 동작을 보존하고, 빠른 단위 테스트와 향후 하이브리드 추출 비교에
활용하기 위해 남겨두었습니다.
...
사용해 LLM에 전달할 상품 context를 만듭니다. 또한 완화 가능한 부가 조건 목록을
기반으로 조건을 하나씩만 완화해 보고, 현재 최선 상품보다 선택 금리가 높은 경우만
별도 대안 context로 만듭니다. 이때 금리 기준은 사용자의 `rate_preference`에 따라
기본금리 또는 최고우대금리를 그대로 따릅니다.

### `graph_chat.py`

LangGraph 기반 멀티턴 챗봇입니다.
LLM 조건 추출 노드, 부족 정보 확인 노드, 상품 검색 노드, OpenAI 답변 생성 노드를
순서대로 실행합니다. 필수 조건이 부족하면 추가 질문을 하고, 조건이 모이면 검색된
상품 정보와 코드가 계산한 조건 완화 대안만으로 최종 답변을 생성합니다. LLM과
prompt는 시작 시 한 번 준비해 대화 턴마다 재사용하며, 콘솔 실행 진입점도 이
파일에 있습니다.
